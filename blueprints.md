...
## ブループリントとは何か
ブループリントとはビュー、テンプレート、静的ファイルなど、アプリケーションに適用される要素をまとめる機能です。
...
機能別構造はこの様なプロジェクトを整理することに向いています。
...
これによりテンプレートの中で必要な情報を参照できるので、ビューでやるべきことはテンプレートをレンダリングするだけです。
...
恐らくテンプレートや静的ファイルを共有することは無いでしょう、という訳で部門別構造で行きましょう。
...
- 5段階の手順によってブループリントを利用するアプリケーションに書き換えることができます。
